...
const RatingItem = (props: RatingItemProps) => {
  ...
  const {track_order_dispatch} = useContext(TrackOrderContext);
  const maxRating = [1, 2, 3, 4, 5];

  const handleRatingChange = (newRating: number) => {
    props.setRating(newRating);
    if (global_state.current_order) {
      const ratings = global_state.current_order.items.map(item => {
        return {
          itemID: item.id,
          rating: newRating,
        };
...
export const RatingPage = (props: RatingPageProps) => {
  ...
  return (
    <PageLayout
      ...
      subHeader={'Help us improve our service by rating your order from ' + 'Chapters Cafe'}
      footer={{
      ...
      <View style={styles.container}>
        <View style={styles.detailsContainer}>
          <RatingItem rating={rating} setRating={setRating} cafe={global_state.current_order?.cafeID} />
