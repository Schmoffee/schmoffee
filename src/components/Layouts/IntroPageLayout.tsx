            ...
            transform: [
                {
                    scale: withDelay(
                        0,
                        withRepeat(withTiming(textBreathe.value, { duration: 700 }), 0, true)
                    ),
    ...
    return (
        ...
        >
            ...
            <Animated.View style={[styles.textContainer, rTextStyle]}>
                <Heading size='large' weight={isIndexZero ? 'Extrabld' : isIndexOne ? 'Bold' : 'Black'} color={Colors.darkBrown} style={styles.text}>{props.title}</Heading>
            </Animated.View>
            {isIndexTwo ?
                (
                    <Animated.View style={[styles.startButton, rBreatheTextStyle]}>
                        <TouchableOpacity onPress={() => navigation.navigate(CONST_SCREEN_SIGNUP)}>
                            <Body size='large' weight='Black' color={Colors.blue} style={styles.start}>let's go</Body>
                        </TouchableOpacity>
                    </Animated.View>
                ) : null}
        </Animated.View>
